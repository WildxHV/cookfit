...
### 2026-05-31 — Cook flow grows the catalog (persist user ingredients + suggested dishes)

User: the dishes Gemini suggests should be added to our DB, and any ingredient a user types that we don't have should also be fetched from Gemini and stored — so the catalog keeps growing from real usage.

- `api/v1/cook.py`: `POST /cook/suggest` now schedules **FastAPI BackgroundTasks** (run after the response, so the user isn't blocked):
  - `_persist_ingredient_bg(term)` — for each user term that matched no catalog ingredient (`_unmatched_terms`, threshold 0.8): dedup-guard (skip if a ≥0.85 match already exists) → `ai_lookup.lookup_ingredient` → `validate_ingredient` → `upsert_ai_ingredient` → commit.
  - `_persist_recipe_bg(name)` — for each suggested dish name (capped at 6): dedup-guard → `ai_lookup.lookup_recipe` → `validate_recipe` → `upsert_ai_recipe` (which also upserts all the recipe's sub-ingredients with nutrition) → commit.
  - Both reuse the exact strict validators as the `/ingredients/ai` and `/recipes/ai` endpoints (so "no false entries" still holds), never raise (errors swallowed/logged), and only run when `ai_enabled`.
- Rationale for re-fetching recipes (vs. storing the lightweight suggestion): a catalog Recipe needs structured ingredients with per-100g nutrition + resolvable units to compute its macros; `lookup_recipe` returns that full structure, validated.
- **Verified live** (ingredients: zucchini, paneer, pasta): background-stored ingredient `zucchini`; `_persist_recipe_bg('Creamy Zucchini Paneer Pasta')` → stored recipe (source=ai, 13 items incl. Pasta (Penne)/Zucchini/Paneer/Cooking Cream, 701 kcal/serving for 2) and is now fetchable at `/recipes/creamy-zucchini-paneer-pasta`. A bad suggestion ("Zucchini Paneer Bhurji") was correctly **rejected** because a sub-ingredient's macros failed the Atwater check — validation gates writes as intended. `pytest` 39 passed.
- **Note / caveat:** each suggest can fire up to ~8 background Gemini calls; on the free tier this can hit the daily quota. The dedup guard means once something is stored it isn't re-fetched, and failures degrade silently (the user still got their suggestions). Background tasks run sequentially and `gemini-2.5-flash` is not instant, so newly-suggested recipes may take a minute to appear in the catalog.

### 2026-05-31 — Batch the cook-flow catalog calls (8 → 2)

User: don't make ~8 background Gemini calls — club the unknown-ingredient lookups into one call, and the suggested-recipe lookups into one call.

- `services/ai_lookup.py`:
  - `_call_gemini` gained an optional `max_output_tokens`; when set (batch mode) it also sends `thinkingConfig.thinkingBudget=0` and a longer HTTP timeout. (gemini-2.5-* are "thinking" models — without disabling thinking, a multi-recipe batch spends its token budget thinking and truncates the per-ingredient `ingredients` arrays, which then fail validation. Disabling thinking + a big output budget fixes it.)
  - New `lookup_ingredients(queries)` → ONE call, `{ingredients:[...]}` (maxOutputTokens 8192). New `lookup_recipes(queries)` → ONE call, `{recipes:[...]}` (maxOutputTokens 32768). Batch prompts are built by reusing the single-item rule text (`Rules:` split) with the `"{query}"` references neutralized.
